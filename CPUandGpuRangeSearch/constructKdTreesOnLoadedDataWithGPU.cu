	...
	for (int i = 0; i<5; i++)
	{
		for (int j = 0; j<ProteinData.ProteinDataHolder[i].heldEntries; j++)
		{
		 if(j%200==0)
			...
			//print3ArraysToFile(kdTreeConstructionArtifacts.xValues, kdTreeConstructionArtifacts.yValues, kdTreeConstructionArtifacts.zValues, ProteinData.ProteinDataHolder[i].proteinLengthCounts[j]);
...
//	std::cout<<"at beginning"<<std::endl;
	dataLoaderWithGpuExtensions kdTreeConstructionArtifacts;
	cudaStream_t streams[3];
	int maxAtomCountSize = 16384;
	cudaStreamCreate(&streams[0]);
	cudaStreamCreate(&streams[1]);
	cudaStreamCreate(&streams[2]);
	kdTreeConstructionArtifacts.xValues = new int[maxAtomCountSize * 10];
	kdTreeConstructionArtifacts.yValues = new int[maxAtomCountSize * 10];
	kdTreeConstructionArtifacts.zValues = new int[maxAtomCountSize * 10];
	kdTreeConstructionArtifacts.xBackPositionReferenceList = new int[maxAtomCountSize * 10];
	kdTreeConstructionArtifacts.yBackPositionReferenceList = new int[maxAtomCountSize * 10];
	kdTreeConstructionArtifacts.zBackPositionReferenceList = new int[maxAtomCountSize * 10];
	kdTreeConstructionArtifacts.xForwardPositionReferenceList = new int[maxAtomCountSize * 10];
	kdTreeConstructionArtifacts.yForwardPositionReferenceList = new int[maxAtomCountSize * 10];
	kdTreeConstructionArtifacts.zForwardPositionReferenceList = new int[maxAtomCountSize * 10];
	kdTreeConstructionArtifacts.kdTreeholder = new int[maxAtomCountSize * 10 * 2];
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_xBackPositionReferenceList, sizeof(int) * maxAtomCountSize * 10));
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_yBackPositionReferenceList, sizeof(int) * maxAtomCountSize * 10));
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_zBackPositionReferenceList, sizeof(int) * maxAtomCountSize * 10));
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_xForwardPositionReferenceList, sizeof(int) * maxAtomCountSize * 10));
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_yForwardPositionReferenceList, sizeof(int) * maxAtomCountSize * 10));
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_zForwardPositionReferenceList, sizeof(int) * maxAtomCountSize * 10));
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_xCoordsHolder, sizeof(int) * maxAtomCountSize * 10));
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_yCoordsHolder, sizeof(int) * maxAtomCountSize * 10));
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_zCoordsHolder, sizeof(int) * maxAtomCountSize * 10));
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_xForwardPositionReferenceList, sizeof(int) * maxAtomCountSize * 10));
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_kdTree, sizeof(int) * maxAtomCountSize * 10));

	for (int i = 0; i<5; i++)
	{
		for (int j = 0; j<ProteinData.ProteinDataHolder[i].heldEntries; j++)
		{
		//	std::cout<<"Processing unit: "<< j<<std::endl;
		//	std::cout<<"Max entry size: "<< ProteinData.ProteinDataHolder[i].MaxEntrySize<<std::endl;
		  if(j%200==0)
                        std::cout<<"-"<<std::endl;
		...
		//	  for(int p=0;p<1000/*(ProteinData.ProteinDataHolder[i].proteinLengthCounts[j] - 1)/2*/;p++)
                  //      {
                    //            if(kdTreeConstructionArtifacts.xValues[p]!=0)
                      //          std::cout<<p<<": "<<kdTreeConstructionArtifacts.xValues[p]<<std::endl;
                        //}
			...
			gpuErrchk(cudaMemsetAsync(kdTreeConstructionArtifacts.d_xCoordsHolder, 999999, (ProteinData.ProteinDataHolder[i].proteinLengthCounts[j] - 1)*4, streams[0]));
			gpuErrchk(cudaMemsetAsync(kdTreeConstructionArtifacts.d_yCoordsHolder, 999999, (ProteinData.ProteinDataHolder[i].proteinLengthCounts[j] - 1)*4, streams[1]));
			gpuErrchk(cudaMemsetAsync(kdTreeConstructionArtifacts.d_zCoordsHolder, 999999, (ProteinData.ProteinDataHolder[i].proteinLengthCounts[j] - 1)*4, streams[2]));
			Parallel_Device_Side_bitonic_sort(kdTreeConstructionArtifacts.xValues, kdTreeConstructionArtifacts.xBackPositionReferenceList, kdTreeConstructionArtifacts.d_xForwardPositionReferenceList, kdTreeConstructionArtifacts.d_xBackPositionReferenceList, kdTreeConstructionArtifacts.d_xCoordsHolder, kdTreeConstructionArtifacts.yValues, kdTreeConstructionArtifacts.yBackPositionReferenceList, kdTreeConstructionArtifacts.d_yForwardPositionReferenceList, kdTreeConstructionArtifacts.d_yBackPositionReferenceList, kdTreeConstructionArtifacts.d_yCoordsHolder, kdTreeConstructionArtifacts.zValues, kdTreeConstructionArtifacts.zBackPositionReferenceList, kdTreeConstructionArtifacts.d_zForwardPositionReferenceList, kdTreeConstructionArtifacts.d_zBackPositionReferenceList, kdTreeConstructionArtifacts.d_zCoordsHolder, blocks, threads);

			//cudaDeviceSynchronize();
			int* numOfAtoms = (int*)malloc(sizeof(int));
			numOfAtoms[0]= ProteinData.ProteinDataHolder[i].proteinLengthCounts[j];
			//std::cout<<"Number of atoms: "<<numOfAtoms[0]<<std::endl;	
				
			//int* ALocal = (int*)malloc(sizeof(int)*8191);
			//int* BLocal = (int*)malloc(sizeof(int)*8191);
			//int* CLocal = (int*)malloc(sizeof(int)*8191);


			 //gpuErrchk(cudaMemcpy(BLocal, kdTreeConstructionArtifacts.d_xBackPositionReferenceList,(ProteinData.ProteinDataHolder[i].proteinLengthCounts[j] - 1)*4 , cudaMemcpyDeviceToHost));
			 //gpuErrchk(cudaMemcpy(ALocal, kdTreeConstructionArtifacts.d_xCoordsHolder,(ProteinData.ProteinDataHolder[i].proteinLengthCounts[j] - 1)*4 , cudaMemcpyDeviceToHost));
			 //gpuErrchk(cudaMemcpy(CLocal, kdTreeConstructionArtifacts.d_xForwardPositionReferenceList,(ProteinData.ProteinDataHolder[i].proteinLengthCounts[j] - 1)*4 , cudaMemcpyDeviceToHost));
//			int aa=3;
			
		//	for(int p=0;p</*ProteinData.ProteinDataHolder[i].proteinLengthCounts[j] - 1*/numOfAtoms[0];p++)
		//	{ 	
//if((ALocal[p]==1061109567)||(ALocal[p]==999999))
//aa=-1; 
				//if((ALocal[p]!=0)&&(ALocal[p]!=999999))
//				std::cout<<p<<": "<<ALocal[p]<<" \t "<<BLocal[p]<<" \t "<<CLocal[p]<<std::endl;
//			}
		//	 std::cout<<"Number of atoms: "<<numOfAtoms[0]<<std::endl;
...
//	std::cout<<"number of levels in kdtree: "<<numberOfLevels<<std::endl;
...
{			online_kdConstructor_slave_nodeX << <  blockcount, threadcount >> >(d_xBackPositionReferenceList, activeTasks, d_kdArray, d_xPositionReferenceList, d_yPositionReferenceList, d_zPositionReferenceList);
 int* localKdTemp = (int*)malloc(sizeof(int)*16383);
 gpuErrchk(cudaMemcpy(localKdTemp, d_kdArray, 4000*sizeof(int), cudaMemcpyDeviceToHost));
...
void GPUTreeWithCPULoad(ProteinDataHandler &ProteinData)
{
	...
	kdTreeConstructionArtifacts.xValues = new int[16384 * 10*2];
	kdTreeConstructionArtifacts.yValues = new int[16384 * 10*2];
	kdTreeConstructionArtifacts.zValues = new int[16384 * 10*2];
	kdTreeConstructionArtifacts.xBackPositionReferenceList = new int[16384 * 10*2];
	kdTreeConstructionArtifacts.yBackPositionReferenceList = new int[16384 * 10*2];
	kdTreeConstructionArtifacts.zBackPositionReferenceList = new int[16384 * 10*2];
	kdTreeConstructionArtifacts.xForwardPositionReferenceList = new int[16384 * 10*2];
	kdTreeConstructionArtifacts.yForwardPositionReferenceList = new int[16384 * 10*2];
	kdTreeConstructionArtifacts.zForwardPositionReferenceList = new int[16384 * 10*2];
	kdTreeConstructionArtifacts.kdTreeholder = new int[16384 * 10 * 2*2];
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_xBackPositionReferenceList, sizeof(int) * 16384 * 10*2));
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_yBackPositionReferenceList, sizeof(int) * 16384 * 10*2));
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_zBackPositionReferenceList, sizeof(int) * 16384 * 10*2));
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_xForwardPositionReferenceList, sizeof(int) * 16384 * 10*2));
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_yForwardPositionReferenceList, sizeof(int) * 16384 * 10*2));
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_zForwardPositionReferenceList, sizeof(int) * 16384 * 10*2));
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_xCoordsHolder, sizeof(int) * 16384 * 10*2));
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_yCoordsHolder, sizeof(int) * 16384 * 10*2));
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_zCoordsHolder, sizeof(int) * 16384 * 10*2));
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_xForwardPositionReferenceList, sizeof(int) * 16384 * 10*2));
	gpuErrchk(cudaMalloc((void**)&kdTreeConstructionArtifacts.d_kdTree, sizeof(int) * 16384 * 10*2));
	...
	//Load data of next protein and reset intermediate value arrays
			...
			gpuErrchk(cudaMemcpy(kdTreeConstructionArtifacts.kdTreeholder, kdTreeConstructionArtifacts.d_kdTree, ProteinData.ProteinDataHolder[i].KdTreeSize*sizeof(int), cudaMemcpyDeviceToHost));
		//	print1ArraysToFile(kdTreeConstructionArtifacts.kdTreeholder, ProteinData.ProteinDataHolder[i].KdTreeSize*sizeof(int));
			//construct kdTree
		//	kdTree_constructor(kdTreeConstructionArtifacts.kdTreeholder, kdTreeConstructionArtifacts.xForwardPositionReferenceList, kdTreeConstructionArtifacts.xBackPositionReferenceList, kdTreeConstructionArtifacts.yForwardPositionReferenceList, kdTreeConstructionArtifacts.yBackPositionReferenceList, kdTreeConstructionArtifacts.zForwardPositionReferenceList, kdTreeConstructionArtifacts.zBackPositionReferenceList, ProteinData.ProteinDataHolder[i].proteinLengthCounts[j], ProteinData.ProteinDataHolder[i].MaxEntrySize);
...
__global__ void online_kdConstructor_slave_nodeX(int * d_xBackPositionReferenceList, int totalNumOfConsecutiveTasks, int* kdArray, int* d_xPositionReferenceList, int*d_yPositionReferenceList, int* d_zPositionReferenceList)
{
	...
	if (id < totalNumOfConsecutiveTasks)
	{
		kdTask task = taskList[id];
			if ((task.kdDestination>0)&&(task.kdDestination<32767))
		{
			int activeElementCount = 0;
			int secondCount = -1;
			int elementToBeExtracted = -1;
