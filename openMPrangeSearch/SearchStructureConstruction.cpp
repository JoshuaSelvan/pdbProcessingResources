...
void searchEntryInSecondaryPositionStructureForAtom(short soughtAtomNum, int EntryNum, int maxSizeOfChains, int* compositionCountsList, int*compositionListArray, int*compositionPointerArray, int*outPutArray, int*outputCount)
{
	...
	int numOfMatches = 0;
	//#pragma omp parallel for shared(compositionListArray) private(inEntryAtomTypeNum,startPosOffSet,numOfMatches)
...
void constructKdTreesOnLoadedDataOnCPUOMP(ProteinDataHandler &ProteinData)
{
        ...
        for (int i = 0; i<5; i++)
        {
                ...
                for (int j = 0; j<ProteinData.ProteinDataHolder[i].heldEntries; j++)
                {
                        ...
                        int *xValues = new int[16384 * 10];
                        int *yValues = new int[16384 * 10];
                        int *zValues = new int[16384 * 10];
                        int *xBackPositionReferenceList = new int[16384 * 10];
                        int *yBackPositionReferenceList = new int[16384 * 10];
                        int *zBackPositionReferenceList = new int[16384 * 10];
                        int *xForwardPositionReferenceList = new int[16384 * 10];
                        int *yForwardPositionReferenceList = new int[16384 * 10];
                        int *zForwardPositionReferenceList = new int[16384 * 10];
                        int *kdTreeholder = new int[16384 * 10 * 2];
                        ...
                        for (int y = 0; y<ProteinData.ProteinDataHolder[i].MaxEntrySize; y++)
                        {
                                 xValues[y] = ProteinData.ProteinDataHolder[i].xCoordsSets[y + ProteinData.ProteinDataHolder[i].MaxEntrySize*j];
                                 yValues[y] = ProteinData.ProteinDataHolder[i].yCoordsSets[y + ProteinData.ProteinDataHolder[i].MaxEntrySize*j];
                                 zValues[y] = ProteinData.ProteinDataHolder[i].zCoordsSets[y + ProteinData.ProteinDataHolder[i].MaxEntrySize*j];

                                 xBackPositionReferenceList[y] = y;
                                 yBackPositionReferenceList[y] = y;
                                 zBackPositionReferenceList[y] = y;
                                 xForwardPositionReferenceList[y] = y;
                                 yForwardPositionReferenceList[y] = y;
                                 zForwardPositionReferenceList[y] = y;
                        }
                        std::fill( kdTreeholder,  kdTreeholder + ProteinData.ProteinDataHolder[i].MaxEntrySize*2, -1);



                        sortReferenceArrayByQuickSort( xValues,  xBackPositionReferenceList, 0, (ProteinData.ProteinDataHolder[i].proteinLengthCounts[j] - 1));
                        setforwardReferenceArrayFromBack( xForwardPositionReferenceList,  xBackPositionReferenceList, ProteinData.ProteinDataHolder[i].proteinLengthCounts[j]);

                        sortReferenceArrayByQuickSort( yValues,  yBackPositionReferenceList, 0, (ProteinData.ProteinDataHolder[i].proteinLengthCounts[j] - 1));
                        setforwardReferenceArrayFromBack( yForwardPositionReferenceList,  yBackPositionReferenceList, ProteinData.ProteinDataHolder[i].proteinLengthCounts[j]);

                        sortReferenceArrayByQuickSort( zValues,  zBackPositionReferenceList, 0, (ProteinData.ProteinDataHolder[i].proteinLengthCounts[j] - 1));
                        setforwardReferenceArrayFromBack( zForwardPositionReferenceList,  zBackPositionReferenceList, ProteinData.ProteinDataHolder[i].proteinLengthCounts[j]);

			//Test if this is where it breaks by commenting it out
                        kdTree_constructor( kdTreeholder,  xForwardPositionReferenceList,  xBackPositionReferenceList,  yForwardPositionReferenceList,  yBackPositionReferenceList,  zForwardPositionReferenceList,  zBackPositionReferenceList, ProteinData.ProteinDataHolder[i].proteinLengthCounts[j], ProteinData.ProteinDataHolder[i].MaxEntrySize);


                        for (int y = 0; y<ProteinData.ProteinDataHolder[i].MaxEntrySize * 2; y++)
                        {
				
                                ProteinData.ProteinDataHolder[i].kdTrees[y + j* ProteinData.ProteinDataHolder[i].KdTreeSize] =  kdTreeholder[y];

                        }
			free (xValues);
			free (yValues);
			free (zValues);
			free (xBackPositionReferenceList);
			free (yBackPositionReferenceList);
			free (zBackPositionReferenceList);
			free (xForwardPositionReferenceList);
			free (yForwardPositionReferenceList);
			free (zForwardPositionReferenceList);
			free (kdTreeholder);
