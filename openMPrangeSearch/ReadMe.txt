This project contains code to run brute force and kd-tree range searches on unzipped PDB files with openMP implemented algorithms.

If you have downloaded or recieved this project from elsewhere, you may have to download some standard libraries to get the makefile to run. This project requires the c++ library boost to be installed on the system and was built with the g++ compiler V 4.8.5. This project also makes use of the ESBTL thrd party library but that should already be present within a "thirdParty" folder within the project.
...
Once compiled, the program can be run by typing in ./openMPRangeSearchRunner.

You can set the amount of threads for openMP to use by typing in:
...
You may select the algorithm you wish to run from within the runDetails.txt by selecting a number in the demarkated line. The main algorithms are numbers 1,2.
