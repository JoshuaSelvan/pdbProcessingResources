...
void ProteinDataHandler::loadAllProteinsToArrays(std::string inputFileLocationsList, AtomToNumHashTable &atomReferenceTable, rangeSearchSettings &currentSettings,int personalFilesToProcess, int personalStartingPointInList)
{
	int debugLvl = currentSettings.debugLevel;
    std::ifstream fileLoader;
	...
	for(int i=0; i<personalStartingPointInList;i++)
		(fileLoader >> currentFile);



	while ((fileLoader >> currentFile) && (filesLoaded<personalFilesToProcess))
	{
		filesLoaded = filesLoaded + 1;
		loadSingleProteinDetailsIntoArrays(currentFile, atomReferenceTable,currentSettings);
		std::cout<<"Processing file No: "<<filesLoaded<<std::endl;
		if (debugLvl > 1)
		{
			if (filesLoaded > 0)
				std::cout << filesLoaded << std::endl;
           ...
           if (systems.empty() || systems[0].has_no_model()){
            ...
            else
            {
                    ...
                    if(numOfAtomsInCurrentProtein!=0)
                    {
                       ...
                       {
						   std::cout << "file of length: " << numOfAtomsInCurrentProtein << " did not fall in valid range and was not inserted into storage"<< std::endl;
                           return 0; 
                       }
                    }
            }
